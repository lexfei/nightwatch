...
func (a *action) request(u *url.URL, params map[string]string) error {
	...
	values.Set("version", nightwatch.Version)
	data := values.Encode()
...
func construct(params map[string]interface{}) (actions.Actor, error) {
	...
	method, err := nightwatch.GetString("method", params)
	switch err {
	case nil:
	case nightwatch.ErrNoKey:
	...
	agent, err := nightwatch.GetString("agent", params)
	switch err {
	case nil:
	case nightwatch.ErrNoKey:
		agent = "nightwatch." + nightwatch.Version
	default:
		return nil, err
	}
	header, err := nightwatch.GetStringMap("header", params)
	switch err {
	case nil:
	case nightwatch.ErrNoKey:
	...
	formParams, err := nightwatch.GetStringMap("params", params)
	if err != nil && err != nightwatch.ErrNoKey {
		return nil, err
	}
